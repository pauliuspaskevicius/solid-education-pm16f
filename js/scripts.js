$(document).ready(function() {
    ...
    $(window).scroll(function() {
        if ($(window).scrollTop() > 0) {
            $(':not(".transparent") > nav').addClass('fixed');
        } else {
            $(':not(".transparent") > nav').removeClass('fixed');
        }
    });
...
$('').find('li').bind('hover, mouseover, mouseenter, click', function(e){
	e.stopPropagation();
	clearInterval(timerId);
});
